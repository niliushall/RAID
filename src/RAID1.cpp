...
int RAID1::getstate() {
    ...
    for (iter = disklist.begin(); iter != disklist.end(); iter++) {
        if ((*iter)->getstate() != DISKSTATE_READY) {
            ...
            cout << "Disk(" << (*iter)->getname() << ") in RAID1(" << getname() << ") is not ready, please check again!"
                 << endl;
            break;
        }
    }
...
int RAID1::addressMapping(int offset, size_t len) {
    ...
    if (slidesize - offset % slidesize >= (int) len)//write to the last disk and only use the one disk
    {
        ...
        while (towrite > 0) {
            diskaddr.diskno = ((++slideno) % disknum) * 2;//д������һ��
            diskaddr_copy.diskno = diskaddr.diskno + 1;
            diskaddr.offset = slideno / disknum * slidesize;//���̵�ƫ����
            diskaddr_copy.offset = diskaddr.offset;
            if (towrite < slidesize)//д���ݵ�д���һ�飨��ʣ�ࣩ
            {
    ...
    for (iter = diskaddrlist.begin(), iter_copy = diskaddrlist_copy.begin();
         iter != diskaddrlist.end(), iter_copy != diskaddrlist_copy.end(); iter++, iter_copy++) {
        DISKADDR *ptr = &(*iter);
        DISKADDR *ptr_copy = &(*iter_copy);
        cout << "AddrMapping: diskno(" << ptr->diskno << "),offset(" << ptr->offset << "),len(" << ptr->len << ")."
             << endl;
        cout << "AddrMapping: diskno(" << ptr_copy->diskno << "),offset(" << ptr_copy->offset << "),len("
             << ptr_copy->len << ")." << endl;
    }


    return diskaddrlist.size(); //д�˶��ٿ����ݣ���������������
}
...
size_t RAID1::operatedisk(int diskno, int offset, void *buf, size_t count, int opflag) {
    switch (opflag) {
        case OP_READ:
            ...
            return getDisk(diskno)->read(offset, buf, count); //��ʱ��read��write��ָ���̲�����
        case OP_WRITE:
